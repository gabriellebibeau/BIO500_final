...
abstract: |
  Dans le cadre d’un suivi de la biodiversité benthique, cette étude évalue l’impact de différentes variables environnementales sur la richesse spécifique de petits écosystèmes lotiques, les rivières et ruisseaux. Nous tentons d’évaluer si la profondeur du ruisseau, sa largeur, la température de son eau et sa transparence ainsi que la vitesse du courant sont corrélées avec la diversité d’espèces présentes dans le milieu. Nos tests statistiques ont révélés que seule la température influençait significativement la richesse spécifique, mais que des cours d’eau aux conditions similaires semblaient également présentés une diversité d’espèces plus semblable par rapport à des ruisseaux aux conditions différentes.
...
```{r, echo=FALSE}
donnees_richesse <- tar_read(table_richesse)
source('fonct_graph_rich.R')
graph_rich(donnees_richesse)
```
...
```{r, echo=FALSE}
donnees_abondance <- tar_read(table_abondance)
source('fonct_ordination_sites.R')
ordination_sites(donnees_abondance)
```
